...
std::vector<std::pair<int, int>> world::get_room_1(int room_width, int room_height) {
    ...
    auto left_wall = shape::make_rectangle(0, 0, wall_size, room_height);
    room.insert(room.end(), left_wall.begin(), left_wall.end());

    auto right_wall = shape::make_rectangle(room_width - wall_size, 0, wall_size, room_height);
    ...
    auto bottom_separator = shape::make_rectangle((room_width / 2) - wall_size, 0, wall_size, room_height / 100 * 40);
    room.insert(room.end(), bottom_separator.begin(), bottom_separator.end());

    auto top_separator = shape::make_rectangle((room_width / 2) - wall_size, room_height - (room_height / 100 * 40), wall_size, room_height / 100 * 40);
